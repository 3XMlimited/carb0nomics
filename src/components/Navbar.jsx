...
const Navbar = ({ currentRoute, loginStatus, setLoginStatus }) => {
  ...
  return (
    <div className={`sticky top-0 z-50 h-[60px] w-full bg-white justify-center ${(currentRoute === 'thankyou' || currentRoute === 'unsuccessful') ? 'hidden' : 'flex'}`}>
      {loginStatus.login ? (
        <div className='relative h-full w-full max-w-[1700px] flex items-center justify-between px-[20px]'>
          ...
          <div className='h-full flex items-center xl:hidden'>
          ...
          <div className='h-full items-center justify-center cursor-pointer hidden xl:flex' onClick={() => setDisplayMenu(!displayMenu)}>
            <FiMenu size={30}/>
          </div>

          <div className={`absolute top-full left-0 h-[calc(100vh-60px)] w-full origin-top-right duration-200 hidden xl:flex ${displayMenu ? 'scale-100 opacity-100' : 'scale-0 opacity-0'}`}>
